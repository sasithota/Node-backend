...
.post(async(req,res,next)=>{
    ...
    if(error) return res.status(200).send({user:null,token:null,error:error.details[0].message});
    // checking existance of user in db 
    try{
       const user = await User.findOne({username:req.body.username});
       if(!user) return res.status(200).send({user:null,token:null,error:"wrong username or password"});
       // compare hashed password and form password with bcyrpt
       const correctPass = await bcrypt.compare(req.body.password,user.password);
       if(!correctPass) return res.status(200).send({user:null,token:null,error:"wrong password"});
       // creating authentication token with jsonwebtoken
       const token = await jwt.sign({u_id : user.id},process.env.JWT_TOKEN_KEY);
       return res.status(200).send({user:user.username,token:token,error:null});
    }catch(e){
