...
const {loginValidator,postValidator} = require('../../authentication/formValidation.js');
...
const Login = (body)=>{
	return new Promise(async(resolve,reject)=>{
		...
		try{
			...
			if(!comparedPass) return reject('username or password is incorrect');
...
const fetchPostsRelated = (id)=>{
	return new Promise(async(resolve,reject)=>{
		...
		try{
			const posts = await Post.find({"author.authorId":{$in : Following}}).sort({createdOn:-1});
            return resolve(posts);
		}catch(e){
...
const getAComment = (id)=>{
	return new Promise(async(resolve,reject)=>{
		try{
          ...
          if(!comment) return reject('comment not found or might be deleted');
...
const updateAComment = (id,content)=>{
      return new Promise(async(resolve,reject)=>{
      	...
      	try{
           const updatedComment = await Comment.update({_id:id},{$set:{content}});
